...
const isValidPath = existsSync;
const writeToFile = (outputFilename, content = []) => writeFileSync(outputFilename, JSON.stringify(content));
const readFromFile = file => readFileSync(file, ENCODING);
const removeFile = unlinkSync;

const updateFiltered = (collection, query, data, multi) => {
...
const removeFiltered = (collection, query, multi) => {
    // break 2 loops at once -  multi : false
    let i = collection.length - 1;
    loop: for (i; i >= 0; i--) {
        var c = collection[i];
...
const finder = (collection, query, multi) => {
    const retCollection = [];
    let i = collection.length - 1;
    loop: for (i; i >= 0; i--) {
        var c = collection[i];
...
module.exports = { isValidPath, writeToFile, readFromFile, removeFile, updateFiltered, removeFiltered, finder, ObjectSearcher }